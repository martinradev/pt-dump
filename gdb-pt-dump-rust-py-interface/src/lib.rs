...
use pt_dump_lib::search::bytes_search;
...
fn create_memory_view(fd: i32, phys_ranges: &PyList) -> Result<QemuMemoryView, PyErr> {
    ...
    let memory_view = match qemu_memory::QemuMemoryView::new(fd, &ram_ranges, true) {
        Err(err) => return Err(pyo3::exceptions::PyIOError::new_err("Couldn't open QEMU mem fd")),
        Ok(res) => res
    };
    Ok(memory_view)
}

fn pages_to_ranges<PageType: GenericPage + Clone, RangeType: GenericPageRange + From<PageType>>(pages: &Vec<PageType>) -> Vec<RangeType> {
    pages.iter().map(|x| RangeType::from(x.clone())).collect()
}

#[pyfunction]
fn parse_page_table_x86_32(fd: i32, cr3: u64, pae: bool, pse: bool, phys_ranges: &PyList) -> PyResult<PageTableX86> {
...
#[pyfunction]
fn parse_page_table_x86_64(fd: i32, cr3: u64, pae: bool, pse: bool, phys_ranges: &PyList) -> PyResult<PageTableX86> {
    let mut memory_view = create_memory_view(fd, &phys_ranges)?;
...
#[pyfunction]
fn parse_page_table_aarch64(fd: i32, pt_pa: u64, phys_ranges: &PyList) -> PyResult<PageTableAarch64> {
    let mut memory_view = create_memory_view(fd, &phys_ranges)?;
    ...
    if let Ok(pages_ok) = pages {
        Ok(PageTableAarch64::new(pages_to_ranges(&pages_ok), memory_view))
    } else {
        return Err(PyTypeError::new_err("Failed to collect pages"));
    }
}
...
fn filter_page_table_x86(table: &mut PageTableX86, filter: &PyAny) -> PyResult<PageTableX86> {
    ...
    Ok(PageTableX86::new(page_range_filter::filter_x86_ranges(table.get_ranges(), &pt_filter), memory_view))
}

fn search_memory_generic<PageRangeType: GenericPageRange + ToString, PageTableType: PageTable<PageRangeType>>(table: &mut PageTableType, data_py: &PyBytes, max_found: usize) -> PyResult<String> {
    let data : &[u8] = data_py.as_bytes();
    let mut memory_view = table.get_memory_view().clone();
    let ranges = table.get_ranges();
    let search_result = bytes_search::search_memory_generic(&data, ranges, &mut memory_view, max_found);
    ...
    for occ in occs {
        ...
        result_str.push_str(&format!("Found at 0x{:016x} in {}\n", occ.va, saved_range.1));
    }
    Ok(result_str)
}

#[pyfunction]
fn search_memory_x86(table: &mut PageTableX86, data_py: &PyBytes, max_found: usize) -> PyResult<String> {
    search_memory_generic(table, data_py, max_found)
}

#[pyfunction]
fn search_memory_aarch64(table: &mut PageTableAarch64, data_py: &PyBytes, max_found: usize) -> PyResult<String> {
    search_memory_generic(table, data_py, max_found)
}
