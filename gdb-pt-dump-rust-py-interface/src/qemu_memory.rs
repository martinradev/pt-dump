...
impl RamRange {
    pub fn new(gpa_start: usize, gpa_extent: usize, hva: usize) -> Self {
        Self { gpa_start: gpa_start, gpa_extent: gpa_extent, hva: hva }
    }
}
...
impl QemuMemoryView {
    pub fn new(mem_fd: i32, sorted_ram_ranges: &Vec<RamRange>, use_cache: bool) -> Result<Self, Error> { 
    ...
    pub fn find_phys_ranges(&self, gpa_start: usize, gpa_end_excl: usize) -> Option<(usize, usize)> {
        ...
        for i in 0..self.sorted_ram_ranges.len() {
            ...
            if start_result.is_none() && ((ram_range.gpa_start <= gpa_start && ram_range.gpa_start + ram_range.gpa_extent > gpa_start) || (ram_range.gpa_start >= gpa_start))  {
                start_result = Some(i);
            }
...
impl MemoryView for QemuMemoryView {
    ...
    fn read_block_inplace(&mut self, gpa: usize, block_size: usize, block: &mut [u8]) -> Result<(), Error> {
        if self.use_cache {
            let value = self.cache.get(&(gpa, block_size));
            if let Some(value_ok) = value {
        ...
        if let Some((start_index, end_index)) = ranges {
            for index in start_index..=end_index {
                ...
                let max_to_read = std::cmp::min(range.gpa_extent - gpa_offset, block_size - (gpa_start - gpa));
                let va_start = range.hva + gpa_offset;
                let cur_pos = gpa_start-gpa;

                // Ignore error.
                let result = read_inline_block_from_fd(self.mem_fd, va_start, max_to_read, &mut block[cur_pos..]);
            }
...
fn test_qemu_memory_view() {
    ...
    assert_eq!(Some((sorted_ranges.len() - 1, sorted_ranges.len() - 1)), range);
}
...
fn test_memory_read() {
    ...
    assert_eq!(true, pid > 0);
    let sz = 0x10000;
    let base_va = unsafe { nc::mmap(0, sz, nc::PROT_READ | nc::PROT_WRITE, nc::MAP_PRIVATE | nc::MAP_ANONYMOUS, -1, 0) }.unwrap();
    ...
    assert_eq!(vec![2u8; 0x1000], qemuMem.read_block(0x1000, 0x1000).unwrap());
    assert_eq!(vec![3u8; 0x1000], qemuMem.read_block(0x2000, 0x1000).unwrap());
    assert_eq!(true, qemuMem.read_block(0x3000, 0x1000).is_err());
    let tmp = [vec![5u8; 0x1000], vec![6u8; 0x1000], vec![7u8; 0x1000], vec![8u8; 0x1000], vec![9u8; 0x1000]].concat();
    ...
    assert_eq!([vec![0x8; 0x8], vec![0x9; 0x8]].concat(), qemuMem.read_block(0x7ff8, 0x10).unwrap());
}